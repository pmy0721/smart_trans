...
`smart_trans` 是一个交通事故智能分析系统：输入事故现场三帧图片，使用 Qwen3-VL 做并行事实抽取，输出结构化事故记录（JSON），使用 DeepSeek-V3.2 对分析结果进行整合，并基于 RAG 交通法律知识库实现事故定性，最终输出事故原因和引用依据。数据可入库至 FastAPI + SQLite 后端，并通过 React 仪表盘进行可视化展示，同时支持蜂鸣器告警扩展。

本文档基于完整代码走读，围绕事件级主链路，说明端到端数据流和关键模块设计。
...
- **输入侧**：一次上报三帧图片（事故当前帧、事故前1秒图片、事故前3秒图片），立即返回 `job_id`，后台异步处理
- **处理侧**：三帧并行帧级分析 → 事件级汇总（cause/report/key_facts）→ 法规检索定性（含引用校验）
- **输出侧**：结果入库 SQLite；前端详情页显示事故描述与法律条款引用（并支持蜂鸣器扩展）

### 亮点二：LLM 分层协作 + 引用强约束

**传统方案的痛点**：多帧场景直接让 LLM 输出“原因/责任/法则”，容易出现结论抖动、编造细节、条文引用不可信，最终导致难以复盘与工程化。
...
📌
**端到端流程：**
事故图片 → 异步并行分析 → 事件归因 → 法规引用 → 入库 → 看板回溯（可选蜂鸣器告警）
