...
- `beep_mcp_server.py`: 蜂鸣器 MCP Server（SSE，默认 `http://localhost:9009/sse`）。
...
- 想“入库+前端可视化”：需要启动 backend（8000），可选启动 frontend dev（5173）或 build 单端口。
...
- Backend
  ...
  - `SMART_TRANS_CORS_ORIGIN`：CORS 允许来源（默认 `http://localhost:5173`）
- MCP Image Receiver
  - `SMART_TRANS_IMAGE_MCP_HOST`：绑定地址（默认 `0.0.0.0`）
  - `SMART_TRANS_IMAGE_MCP_PORT`：端口（默认 `9010`）
...
- 列表：`GET http://localhost:8000/api/accidents`
- 统计：`GET http://localhost:8000/api/stats/summary`
...
```bash
python3 pipeline_yolo_rag.py -i input_image/image1.jpg --task rag --upload http://localhost:8000/api/uploads --post http://localhost:8000/api/accidents --beep --beep-start-server
```

注意：蜂鸣器默认 MCP URL 是 `http://localhost:9009/sse`，如需修改用 `--beep-mcp-url`。
...
- `http://<server-ip>:9010/sse`

### 2) 在发送机上发送图片（并等待完成）

```bash
python3 send_images_mcp.py --server http://<server-ip>:9010/sse input_image/image1.jpg --wait
```

批量发送（目录）：

```bash
python3 send_images_mcp.py --server http://<server-ip>:9010/sse --dir input_image --wait
```
...
python3 send_images_mcp.py \
  --server http://<server-ip>:9010/sse \
...
- `--beep` 不会默认开启，必须传。
- `--upload/--post` URL 是“接收机视角”的地址：如果 backend 也在接收机本机跑，`localhost:8000` 是正确的。
...
- 客户端 URL 是否为 `http://<server-ip>:9010/sse`
- 防火墙是否允许 9010 入站
...
- `beep_mcp_server.py` 是否可访问（默认 `http://localhost:9009/sse`）
